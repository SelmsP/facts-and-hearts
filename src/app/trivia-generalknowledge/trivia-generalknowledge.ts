...
@Component({
  ...
  templateUrl: './trivia-generalknowledge.html',
  styleUrls: ['./trivia-generalknowledge.css']
})
export class TriviaGeneralknowledge implements OnInit {
  ...
  async loadTrivia() {
    this.isLoading = true;
    const cachedData = localStorage.getItem('generalknowledge_trivia');
    if (cachedData) {
  ...
  async fetchQuestions() {
    this.isLoading = true;
    const url = "https://the-trivia-api.com/v2/questions?limit=10&categories=generalknowledge";
    try {
  ...
  restart() {
    localStorage.removeItem('generalknowledge_trivia');
    this.fetchQuestions();
  }
